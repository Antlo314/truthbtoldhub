...
import { hutCompletion } from './stationProgress';
...
export default function HouseCinematicChrome({
            ...
            <>
                ...
                {hotspot && (
                    <div
                        ...
                        style={{ bottom: 'calc(min(42dvh, 280px) + 0.35rem)' }}
                    >
                        <p className="text-[11px] text-amber-50 font-medium bg-black/70 border border-amber-400/40 px-3 py-1.5 rounded-full backdrop-blur-md max-w-[90vw] text-center shadow-lg">
                            {hotspot.hint}
                        </p>
                    </div>
                )}

                {/* Side vignette for power feel */}
    ...
    return (
        <>
            ...
            {/* Right tools — all in-app, no OS menus */}
            <div
            ...
            <div className="fixed bottom-[calc(4.5vh+1.25rem)] inset-x-0 z-30 flex flex-col items-center gap-2 pointer-events-none">
                {hotspot ? (
                    <button
                        type="button"
                        className="pointer-events-auto px-6 py-3 rounded-full border border-amber-400/55 bg-black/85 text-amber-50 text-sm font-semibold backdrop-blur-md shadow-[0_0_40px_rgba(251,191,36,0.2)] hover:bg-black"
                        onClick={onInteract}
                    >
                        E · {hotspot.hint}
                    </button>
